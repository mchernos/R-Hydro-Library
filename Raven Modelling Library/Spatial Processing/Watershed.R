...
init_value = 50000000			# "my" default = 10000000


# # Load DEM and prepare for SAGA Analyses
# dem = raster('adams_dem/cdem_dem_170105_172807_tif/cdem_dem_170105_172807.tif')
pro =  '+proj=utm +zone=11 +ellps=GRS80 +datum=NAD83 +units=m +no_defs'
# dem = projectRaster(dem, crs = pro, res = 100)
# values(dem) = ifelse(values(dem)<= 0, NA, values(dem))
...
# rsaga.fill.sinks('dem_saga.sgrd', 'permanent_data/dem_sinkfilled.sgrd', method = "xxl.wang.liu.2006")
dem_filled = raster('permanent_data/dem_sinkfilled.sdat')
projection(dem_filled) = projection(crs(pro))
# Remove dead areas
values(dem_filled) = ifelse(values(dem_filled)<= 0, NA, values(dem_filled))
writeRaster(dem_filled, "permanent_data/dem_sinkfilled.sdat", "SAGA", overwrite = T)
...
# Create/Read POIs
POIs = readOGR('permanent_data/AdamsR_jan5.kml', 'AdamsR_jan4')
POIs = spTransform(POIs, crs(dem_filled))
# Flip order for POIs
poi_dat = data.frame(POIs)
POIs = SpatialPointsDataFrame(poi_dat[,3:4], poi_dat, proj4string = crs(dem_filled))
...
subb = gdal_polygonizeR(catch)					# Only need first polygon (2nd is square from dem)
...
# Now plot that shit
plot(dem_filled, col = terrain.colors(100))
plot(subb, add = T)
...
writeOGR(subb[1,],'permanent_data/watersheds_shape.shp',layer = 'watersheds_shape', driver = 'ESRI Shapefile', overwrite = T)
